...
record Participant(String name, String searchingFor, Maze maze, int[] startingPositions) {
    Participant {
        maze.moveLocation(startingPositions[0], startingPositions[1], name);
    }
}
...
class ParticipantThread extends Thread {
    ...
    @Override
    public void run() {
        int[] lastSpot = participant.startingPositions();
        Maze maze = participant.maze();

        while (true) {
            int[] newSpot = maze.getNextLocation(lastSpot);
            try {
                Thread.sleep(participant.name().equals("Adam") ? 2900: 500);
