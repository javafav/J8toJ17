...
record OrderDetails(int orderDetailId, String itemDescription, int qty) {

    public OrderDetails(String itemDescription, int qty) {
...
record Orders(int orderId, String dateString, List<OrderDetails> details) {

    public Orders(String dateString) {
        this(-1, dateString, new ArrayList<>());
    }

    public void addDetail(String itemDescription, int qty) {
        OrderDetails item = new OrderDetails(itemDescription, qty);
        details.add(item);
    }
...
//            addOrders(conn, orders);
    ...
    private static List<Orders> readData() {

        List<Orders> vals = new ArrayList<>();

        try (Scanner scanner = new Scanner(Path.of("files/Orders.csv"))) {
    ...
    private static void addOrder(Connection conn, PreparedStatement psOrder,
                                 PreparedStatement psDetail, Orders order)
            throws SQLException {
            ...
            if (psOrder.executeUpdate() == 1) {
                ...
                if (rs.next()) {
                    ...
                    if (orderId > -1) {
                        ...
                        for (OrderDetails od : order.details()) {
    ...
    private static void addOrders(Connection conn, List<Orders> orders) {
