...
public class Theatre {
    ...
    class  Seat implements Comparable<Seat> {
        ...
        public Seat(int rowChar, int seatNo) {
    ...
    public String reserveSeat(char row, int seat){
       ...
       if(requested == null || !requested.seatNum.equals(requestedSeat.seatNum)){
           System.out.println("--> No Such Seats " + requestedSeat);
           System.out.printf(" : Seats must between %s and %s ", seats.first().seatNum, seats.last().seatNum);
       }else {
           if(!requested.reserved){
               requested.reserved = true;
               return requested.seatNum;
           }else {
               System.out.printf("\nThis Seat %s  is already  reserved.", requested.seatNum );
           }
       }
       return null;
    }
